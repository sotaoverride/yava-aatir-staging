$(document).on('turbolinks:load', function () {
  $('.active-deals').slick({
    infinite: false,       swipe: false,       slidesToShow: 5,       slidesToScroll: 1
  })

  $('.reveal').on('open.fndtn.reveal', function () {
    $('body').addClass('modal-open')
  })
  $('.reveal').on('close.fndtn.reveal', function () {
  ...
  $('#upload-csv .dropify').dropify({
    messages: {
      'default': 'Drag your product template here or <span>browse</span> to upload',       'replace': 'Drag your File Template here or browse to replace it',       'remove': 'Remove',       'error': 'Ooops, something wrong appended.'
    },     tpl: {
      wrap: '<div class="dropify-wrapper"></div>',       loader: '<div class="dropify-loader"></div>',       message: '<div class="dropify-message"><span class="icon-drag-drop" /> <p>{{ default }}</p></div>',       preview: '<div class="dropify-preview"><span class="dropify-render"></span><div class="dropify-infos"><div class="dropify-infos-inner"><p class="dropify-infos-message">{{ replace }}</p></div></div></div>',       filename: '<p class="dropify-filename"><span class="file-icon"></span> <span class="dropify-filename-inner"></span></p>',       clearButton: '<button type="button" class="dropify-clear">{{ remove }}</button>',       errorLine: '<p class="dropify-error">{{ error }}</p>',       errorsContainer: '<div class="dropify-errors-container"><ul></ul></div>'
    }
  })
  $('.dropify-po, #upload-po .dropify').dropify({
    messages: {
      'default': 'Drag your purchase order here<br/> or <span>Browse</span> to upload',       'replace': 'Drag your PO file here<br/> or browse to replace it',       'remove': 'Remove',       'error': 'Ooops, something wrong appended.'
    },     tpl: {
      wrap: '<div class="dropify-wrapper"></div>',       loader: '<div class="dropify-loader"></div>',       message: '<div class="dropify-message"><span class="icon-upload-bill" /> <h4>{{ default }}</h4></div>',       preview: '<div class="dropify-preview"><span class="dropify-render"></span><div class="dropify-infos"><div class="dropify-infos-inner"><p class="dropify-infos-message">{{ replace }}</p></div></div></div>',       filename: '<p class="dropify-filename"><span class="file-icon"></span> <span class="dropify-filename-inner"></span></p>',       clearButton: '<button type="button" class="dropify-clear">{{ remove }}</button>',       errorLine: '<p class="dropify-error">{{ error }}</p>',       errorsContainer: '<div class="dropify-errors-container"><ul></ul></div>'
    }
  })
